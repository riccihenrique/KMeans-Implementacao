...
class KMeans(object):    
    ...
    def train(self):
        ...
        while True:
            # O K + 1 é para adicionar na ultima coluna quem possui a menor distancia1
            self.distancias = []

            for linha in range(len(self.dados)): # para cada linha dos dados
                self.distancias.append([0] * (self.k + 1))
                for centroids_index in range(self.k): # e para cada um dos centroids
                    distancia = 0
                    for coluna in range(len(self.dados[0])): # calcula-se a distancia para cada atributo
